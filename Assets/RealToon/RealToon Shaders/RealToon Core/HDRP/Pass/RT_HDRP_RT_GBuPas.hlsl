...
void ClosestHitGBuffer(inout RayIntersectionGBuffer rayIntersectionGbuffer : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
	...
	float RTD_GLO; 
	float3 RTD_GLO_COL;
	RT_GLO( fragInput.texCoord0.xy , RTD_VD_Cal , halfDirection , normalDirection, viewDirection , DirSpecDim , RTD_GLO , RTD_GLO_COL);
	...
	if (LIGHTFEATUREFLAGS_PUNCTUAL)
	{
		...
		for (i = lightStart; i < lightEnd; ++i)
		{
			...
			if (IsMatchingLightLayer(Plight.lightLayers, builtinData.renderingLayers))
			{
				...
				float RTD_GLO; 
				float3 RTD_GLO_COL;
				RT_GLO( fragInput.texCoord0.xy , RTD_VD_Cal , halfDirection , normalDirection, viewDirection , Plight.specularDimmer , RTD_GLO , RTD_GLO_COL);
				float3 RTD_GLO_OTHERS = RTD_GLO;
			...
			if (NALC > 0)
			{
				...
				while (i <= last )
				{
					...
					if(IsMatchingLightLayer(Alight.lightLayers, builtinData.renderingLayers))
					{
						...
						float RTD_GLO; 
						float3 RTD_GLO_COL;
						RT_GLO( fragInput.texCoord0.xy , RTD_VD_Cal , halfDirection , normalDirection, viewDirection , Alight.specularDimmer , RTD_GLO , RTD_GLO_COL);
	...
	//RT_CA
	float3 RTD_CA = RT_CA( color * GetCurrentExposureMultiplier() );
	...
	rayIntersectionGbuffer.gbuffer3 = float4(finalRGBA.rgb,1.0);
