...
void ClosestHitForward(inout RayIntersection rayIntersection : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
	...
	const float3 rayOri = WorldRayOrigin();
	...
	//float2 RTD_VD_Cal = -(float2((sceneUVs.x * 2 - 1)*(_ScreenParams.r/_ScreenParams.g), sceneUVs.y * 2 - 1).xy * RTD_OB_VP_CAL ); **

	float2 RTD_TC_TP_OO = fragInput.texCoord0.xy; //lerp( fragInput.texCoord0.xy, RTD_VD_Cal, _TexturePatternStyle ); **
	...
	//RT_SCT
	float3 RTD_SCT = RT_SCT( fragInput.texCoord0.xy , RTD_MCIALO_IL );
	...
	float3 RTD_LAS = lerp(ss_col * RTD_LVLC,( ss_col * lig_col_int ),_LightAffectShadow);
	...
	float RTD_GLO; 
	float3 RTD_GLO_COL;
	RT_GLO( fragInput.texCoord0.xy , fragInput.texCoord0.xy , halfDirection , normalDirection, viewDirection , DirSpecDim , RTD_GLO , RTD_GLO_COL); //fragInput.texCoord0.xy -> RTD_VD_Cal **
	...
	float RTD_ST_IS;
	float3 RTD_ST_LAF;
	float RTD_ST = RT_ST ( fragInput.texCoord0.xy , RTD_NDOTL , attenuation , RTD_LVLC , RTD_PT_COL , lig_col_int , RTD_SCT , RTD_OSC , RTD_PT , RTD_SHAT_COL , RTD_STIAL , RTD_ST_IS , RTD_ST_LAF);
	...
	//RT_R
	float3 RTD_R = RT_R( fragInput.texCoord0.xy , viewDirection , normalDirection , EnvRef , RTD_TEX_COL , RTD_R_OFF_OTHERS );

	//RT_SL
	float3 RTD_SL_CHE_1;
	float3 RTD_SL = RT_SL( fragInput.texCoord0.xy , RTD_SL_OFF_OTHERS , RTD_TEX_COL , RTD_R , RTD_SL_CHE_1);
	...
	if (LIGHTFEATUREFLAGS_PUNCTUAL)
	{
		...
		#else
			i = 0;
			i_en = _PunctualLightCountRT;
		#endif

		for (i = 0; i < i_en; ++i)
		{
			LightData Plight = _LightDatasRT[i];
			...
			float RTD_GLO; 
			float3 RTD_GLO_COL;
			RT_GLO( fragInput.texCoord0.xy , fragInput.texCoord0.xy, halfDirection , normalDirection, viewDirection , Plight.specularDimmer , RTD_GLO , RTD_GLO_COL); //fragInput.texCoord0.xy -> RTD_VD_Cal **
			float3 RTD_GLO_OTHERS = RTD_GLO;
			...
			float RTD_ST_IS;
			float3 RTD_ST_LAF;
			float RTD_ST = RT_ST ( fragInput.texCoord0.xy , RTD_NDOTL , lightfos , RTD_LVLC , RTD_PT_COL , lig_col_int , RTD_SCT , RTD_OSC , RTD_PT , RTD_SHAT_COL , RTD_STIAL , RTD_ST_IS , RTD_ST_LAF);
			...
			//RT_SL
			//float3 RTD_SL_CHE_1;
			float3 RTD_SL = RT_SL( fragInput.texCoord0.xy , (float3)0.0 , RTD_TEX_COL , RTD_R , RTD_SL_CHE_1);
	...
	//=========//
		...
		if (LIGHTFEATUREFLAGS_AREA)
		{
			...
			if (NALC > 0)
			{
				...
				#else
					LightData Alight = _LightDatasRT[NPLC+i];
				#endif
				...
				while (i <= last )
				{
					...
					if(IsMatchingLightLayer(Alight.lightLayers, builtinData.renderingLayers))
					{
						...
						float RTD_GLO; 
						float3 RTD_GLO_COL;
						RT_GLO( fragInput.texCoord0.xy , fragInput.texCoord0.xy, halfDirection , normalDirection, viewDirection , Alight.specularDimmer , RTD_GLO , RTD_GLO_COL); //fragInput.texCoord0.xy -> RTD_VD_Cal **
					...
					#else
						Alight = _LightDatasRT[NPLC+(min(++i, last))];
					#endif
	...
	rayIntersection.color = (float3)finalRGBA;
	rayIntersection.rayCount += additionalRayCount;

#ifdef N_F_TRANS_ON

	rayIntersection.color = lerp(transmitted, rayIntersection.color, Trans_Val);
