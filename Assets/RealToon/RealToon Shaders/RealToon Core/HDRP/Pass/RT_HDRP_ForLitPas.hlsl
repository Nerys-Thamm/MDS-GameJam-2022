...
float4 LitPassFragment(Varyings input, float facing : VFACE) : SV_Target
{
	...
	bool bo_co_val;
	float RTD_CO;				
	RT_TRANS_CO( input.uv, _MainTex_var , RTD_TRAN_OPA_Sli , RTD_CO , bo_co_val , false);
	...
	//RT_SCT
	float3 RTD_SCT = RT_SCT( input.uv , RTD_MCIALO_IL );
	...
	float RTD_GLO; 
	float3 RTD_GLO_COL;
	RT_GLO( input.uv , RTD_VD_Cal , halfDirection , normalDirection, viewDirection , DirSpecDim , RTD_GLO , RTD_GLO_COL);
	...
	float RTD_ST_IS;
	float3 RTD_ST_LAF;
	float RTD_ST = RT_ST ( input.uv , RTD_NDOTL , attenuation , RTD_LVLC , RTD_PT_COL , lig_col_int , RTD_SCT , RTD_OSC , RTD_PT , RTD_SHAT_COL , RTD_STIAL , RTD_ST_IS , RTD_ST_LAF);
	...
	//RT_SL
	float3 RTD_SL_CHE_1;
	float3 RTD_SL = RT_SL( input.uv , RTD_SL_OFF_OTHERS , RTD_TEX_COL , RTD_R , RTD_SL_CHE_1);
...
if (LIGHTFEATUREFLAGS_PUNCTUAL)
{
    ...
    for (i = 0; i < i_en; ++i)
    {
		...
		float RTD_GLO; 
		float3 RTD_GLO_COL;
		RT_GLO( input.uv , RTD_VD_Cal , halfDirection , normalDirection, viewDirection , Plight.specularDimmer , RTD_GLO , RTD_GLO_COL);
		float3 RTD_GLO_OTHERS = RTD_GLO;
		...
		float RTD_ST_IS;
		float3 RTD_ST_LAF;
		float RTD_ST = RT_ST ( input.uv , RTD_NDOTL , lightfos , RTD_LVLC , RTD_PT_COL , lig_col_int , RTD_SCT , RTD_OSC , RTD_PT , RTD_SHAT_COL , RTD_STIAL , RTD_ST_IS , RTD_ST_LAF);
		...
		//RT_SL
		//float3 RTD_SL_CHE_1;
		float3 RTD_SL = RT_SL( input.uv , (float3)0.0 , RTD_TEX_COL , RTD_R , RTD_SL_CHE_1);
		...
		if (NALC > 0)
		{
			...
			while (i <= last )
			{
				...
				if(IsMatchingLightLayer(Alight.lightLayers, builtinData.renderingLayers))
				{
					...
					float RTD_GLO; 
					float3 RTD_GLO_COL;
					RT_GLO( input.uv , RTD_VD_Cal , halfDirection , normalDirection, viewDirection , Alight.specularDimmer , RTD_GLO , RTD_GLO_COL);
